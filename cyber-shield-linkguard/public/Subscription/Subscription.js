...
async function runScanUrl(url){
  ...
  try{
    ...
    const vt = j.signals.virustotal || {};
    if (vt.enabled && !vt.error) {
      const vtLi = document.createElement('li');
      vtLi.innerHTML = `<strong>VirusTotal Results:</strong> ${vt.malicious || 0} malicious, ${vt.suspicious || 0} suspicious, ${vt.harmless || 0} harmless, ${vt.undetected || 0} undetected`;
      $('reasonsUrl').appendChild(vtLi);
    }
    ...
    // Update stats based on actual VirusTotal results
    ...
    updateStats(malicious, suspicious, harmless, undetected);
...
async function runScanFile(file){
  ...
  try{
    ...
    let vtSummary = '';
    const vt = j.virustotal || {};
    if (vt.enabled && !vt.error) {
      vtSummary = `Malicious: ${vt.malicious || 0}, Suspicious: ${vt.suspicious || 0}, ` +
                 `Harmless: ${vt.harmless || 0}, Undetected: ${vt.undetected || 0}`;
    } else {
